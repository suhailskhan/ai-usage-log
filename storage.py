...
class SQLiteStorage(Storage):
    ...
    def _ensure_table(self):
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS entries (
            Name TEXT, Manager TEXT, AI_Tool TEXT, Purpose TEXT, Duration INTEGER, Result_Outcome TEXT, Notes TEXT
        )''')
        self.conn.commit()
    def load(self):
        c = self.conn.cursor()
        c.execute("SELECT Name, Manager, AI_Tool, Purpose, Duration, Result_Outcome, Notes FROM entries")
        rows = c.fetchall()
        return [{'Name': r[0], 'Manager': r[1], 'AI Tool': r[2], 'Purpose': r[3], 'Duration': r[4], 'Result/Outcome': r[5], 'Notes': r[6]} for r in rows]
    def save(self, entries):
        c = self.conn.cursor()
        c.execute("DELETE FROM entries")
        for e in entries:
            c.execute("INSERT INTO entries VALUES (?,?,?,?,?,?,?)", (
                e['Name'], e['Manager'], e['AI Tool'], e['Purpose'], e['Duration'], e['Result/Outcome'], e['Notes']
            ))
        self.conn.commit()
